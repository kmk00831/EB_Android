...
const std::string stairsJsonStr = R"({
  "-2": {
    "상승": {
        "90": [[324, 195], [314, 692]]
    }
...
// 4층 "상승" : 5층 모델링 완료되면 5층 좌표계로 수정 필요
json stairsInfo = json::parse(stairsJsonStr);


const std::string ElevatorJsonStr = R"({
  "180": [[312, 760]]
})";

// 4층 "상승" : 5층 모델링 완료되면 5층 좌표계로 수정 필요
json elevatorInfo = json::parse(ElevatorJsonStr);
...
private:
    ...
    int roi_radius = 1000; // ROI 반경 (셀 단위)
...
public:
    ...
    PositionTracker(const BitMatrix& binMap, const MapData& mapData, int start_floor)
            : binary_map(binMap), map_data(mapData), cur_pos({0, 0}),
              has_last_matched(false), currentFloor(start_floor)
    {
    ...
    void setArrivedInfo(float gyro, int elevationMode) {
        ...
        if (currentFloor != previousFloor) {
            ...
            if (elevationMode == 2) {  // elevationMode 2는 엘리베이터
                ...
                if (stairsInfo.contains(to_string(previousFloor)) &&
                    stairsInfo[to_string(previousFloor)].contains(stairElevation)) {
                    json modeInfo = stairsInfo[to_string(previousFloor)][stairElevation];
...
//                ss << "계단 도착! newFloorMode, roi_center On!" << "\n";
            ...
            string floorText = (currentFloor < 0 ? "B" + to_string(abs(currentFloor)) : to_string(currentFloor)) + "F";
            string binaryMapFile = "zeromap_"+ floorText + " - origin(" + to_string(originMap[currentFloor].first) + "," + to_string(originMap[currentFloor].second) + ").txt";
            string indoorMapFile = "indoor_map_" + floorText + ".json";
...
//        nativeMessage = ss.str();
    ...
    ProcessResult processStep(float gyro, float compass, float stepLength, int stepCount, int floor, float arrivedGyroValue, int elevationMode) {
        ...
        // processStep 함수 시작 부분 (캘리브레이션 처리 부분)
        double adjustedCompass = fmod((compass - 339.38 + 360), 360);
        double diffCompassGyro = fabs(adjustedCompass - gyro);
        double diffCompassGyroStepByStep = 0.0;
...
//        }
        ...
        // 최종 위치 업데이트
        cur_pos_global = {static_cast<double>(final_coord.first - originMap[currentFloor].first), static_cast<double>(final_coord.second - originMap[currentFloor].second)};
...
// 원점 정보를 로드하는 함수
void loadOriginOfMap(std::map<int, std::pair<int, int>>& originMap) {
    std::regex pattern(R"(zeromap_((B)?(\d+))F - origin\((\d+),(\d+)\)\.txt)");

    // assets 폴더에서 파일 목록 가져오기
    std::vector<std::string> fileList = listAssetFiles("");
...
extern "C"
JNIEXPORT void JNICALL
Java_com_fifth_maplocationlib_NativeLib_initializeEngine(JNIEnv *env, jobject, jint floor) {
    // 원점 정보 로드
    loadOriginOfMap(originMap);

    __android_log_print(ANDROID_LOG_INFO, "LOG_CHECK", "===== Origin Map Contents =====");
    __android_log_print(ANDROID_LOG_INFO, "LOG_CHECK", "Total entries: %d", originMap.size());

    for (const auto& entry : originMap) {
        __android_log_print(ANDROID_LOG_INFO, "LOG_CHECK", "Floor %d: Origin at (%d, %d)",
                            entry.first,                // 층 번호
                            entry.second.first,         // x 좌표
                            entry.second.second);       // y 좌표
    }
    ...
    string binaryMapFile = "zeromap_"+ floorText + " - origin(" + to_string(originMap[start_floor].first) + "," + to_string(originMap[start_floor].second) + ").txt";
    string indoorMapFile = "indoor_map_" + floorText + ".json";
    ...
    gTracker = new PositionTracker(binaryMap, mapData, start_floor);
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_fifth_maplocationlib_NativeLib_reSearchStart(JNIEnv *env, jobject) {
    gTracker->reSearchStart(cur_pos_global, 100);
}